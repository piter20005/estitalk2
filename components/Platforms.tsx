    ...
    <section id="listen" className="py-24 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {LINKS.map(({ name, url, icon: Icon, desc }) => (
            <a
              key={name}
              href={url}
              target="_blank"
            ...
            >
              ...
              <div>
                <p className="font-sans text-xs uppercase tracking-widest mb-2 text-white/60 font-medium">{desc}</p>
                <h3 className="font-serif text-2xl font-medium text-white">{name}</h3>
              </div>
            </a>
          ))}
        </div>
